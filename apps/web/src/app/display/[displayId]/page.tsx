...
export default function PublicDisplayPage({ params }: { params: Promise<{ displayId: string }> }) {
  ...
  return (
    <div className="w-screen h-screen bg-[#f8fafc] bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-50 via-white to-cyan-50 flex flex-col font-sans text-slate-900 overflow-hidden" onClick={enableAudio}>
      {/* HEADER */}
      <header className="flex justify-between items-center px-6 py-3 2xl:px-8 2xl:py-4 bg-gradient-to-r from-blue-900 via-indigo-900 to-purple-900 shrink-0 shadow-lg z-10 border-b border-white/10">
      ...
      {/* NOW SERVING AREA */}
      <section className="flex-shrink-0 flex px-8 py-[clamp(0.5rem,1.5vh,1.5rem)] border-b border-indigo-100/50 bg-white/40 backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.02)]">
        <div className="w-1/4 flex flex-col items-center justify-center text-center border-r border-indigo-100/50 pr-6">
          <div className={`p-3 2xl:p-4 rounded-2xl mb-1 transition-all duration-500 shadow-lg ${announcementSettings.enabled ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-blue-500/30' : 'bg-slate-100 text-slate-400 shadow-slate-200/50'}`}>
            {announcementSettings.enabled ? (
              <svg className="w-8 h-8 2xl:w-12 2xl:h-12 drop-shadow-md" fill="currentColor" viewBox="0 0 24 24"><path d="M13 5v14l-5-4H3V9h5l5-4zm2.5 7c0-1.7-.9-3.2-2.3-4l-.7 1.4c1 1.6.3 3.6-1.3 4.6l.7 1.4c1.8-1 3-2.9 3-3.4zM18 12c0-3.3-1.8-6.2-4.5-7.7l-.8 1.4c2.1 1.2 3.5 3.5 3.5 6s-1.4 4.8-3.5 6l.8 1.4C16.2 17.6 18 14.8 18 12z" /></svg>
            ) : (
              <svg className="w-8 h-8 2xl:w-12 2xl:h-12 drop-shadow-sm" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" /></svg>
            )}
          </div>
          <h3 className="font-extrabold text-slate-800 text-[clamp(0.875rem,1.5vh,1.125rem)] tracking-tight">Audio Announcement</h3>
          <p className={`font-bold text-[clamp(0.7rem,1.2vh,0.875rem)] mt-0.5 px-3 py-0.5 rounded-full ${announcementSettings.enabled ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-500'}`}>
            {voiceUnavailable ? 'Unavailable' : announcementSettings.enabled ? 'Active' : 'Muted'}
          </p>
        </div>

        <div className="w-1/2 flex flex-col items-center justify-center px-8 relative">
          ...
          <h2 className="text-[clamp(1rem,2vh,1.5rem)] font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 uppercase tracking-[0.3em] z-10 drop-shadow-sm">Now Serving</h2>
          <div className="flex items-center justify-center my-1 z-10">
            <span className={`text-[clamp(3.5rem,12vh,7rem)] leading-none font-black tracking-tighter transition-all duration-500 ${lastToken === current?.tokenLabel ? 'scale-110 text-transparent bg-clip-text bg-gradient-to-br from-indigo-500 via-purple-600 to-pink-500 drop-shadow-2xl' : 'text-slate-800 drop-shadow-lg'}`}>
              {current?.tokenLabel || '—'}
            </span>
          </div>
          {current?.counter && (
            <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white text-[clamp(1rem,2vh,1.5rem)] font-black uppercase tracking-widest px-6 py-1.5 2xl:px-8 2xl:py-2 rounded-xl shadow-[0_8px_30px_rgb(79,70,229,0.3)] z-10 border border-white/20 transform transition-transform hover:scale-105">
              {current.counter}
            </div>
          )}
        </div>

        <div className="w-1/4 flex flex-col items-center justify-center border-l border-indigo-100/50 pl-6 relative">
          ...
          <svg className="w-full max-w-[120px] 2xl:max-w-[160px] drop-shadow-xl z-10" viewBox="0 0 100 80" fill="none">
            <defs>
        ...
        <div className={`grid w-full h-full gap-4 lg:gap-6 min-h-0 ${
          ...
          {counters.map((c, idx) => (
            <div key={c.id || idx} className="@container flex flex-col bg-white rounded-2xl lg:rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden min-h-0 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_40px_rgb(79,70,229,0.12)]">
              <div className="flex-shrink-0 py-1.5 2xl:py-3 flex flex-col items-center justify-center bg-gradient-to-r from-slate-50 to-indigo-50/30 border-b border-indigo-50 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-400 via-indigo-500 to-purple-500"></div>
                <h3 className="font-bold text-slate-500 text-[clamp(0.55rem,1.2vh,0.7rem)] uppercase tracking-[0.2em]">{c.name || 'Counter'}</h3>
                <span className="font-black text-slate-800 text-[clamp(1.1rem,2.5vh,1.35rem)] uppercase tracking-widest mt-0.5">{c.code || c.counter}</span>
              </div>
              <div className="flex-shrink-0 py-2 2xl:py-3 flex flex-col items-center justify-center relative">
                ...
                <p className="font-bold text-blue-600 text-[clamp(0.55rem,1.2vh,0.7rem)] tracking-[0.2em] uppercase mb-1 z-10 bg-blue-50 px-3 py-0.5 rounded-full border border-blue-100">Now Serving</p>
                <p className="font-black text-[clamp(2.5rem,8vh,4.5rem)] leading-none text-transparent bg-clip-text bg-gradient-to-br from-slate-800 to-slate-600 drop-shadow-sm z-10">
                  {c.now?.tokenLabel || '—'}
                </p>
              </div>
              <div className="flex-shrink-0 py-1.5 2xl:py-2.5 flex flex-col items-center justify-center bg-slate-50/80 border-y border-slate-100">
                <p className="font-bold text-purple-600 text-[clamp(0.55rem,1.2vh,0.7rem)] tracking-[0.2em] uppercase mb-0.5">Next</p>
                <p className="font-black text-[clamp(1.75rem,5vh,3rem)] leading-none text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-500">
                  {c.next?.tokenLabel || '—'}
                </p>
              </div>
              <div className="flex-1 flex flex-col bg-white min-h-0 overflow-hidden">
                <div className="py-1 2xl:py-1.5 bg-slate-50/50 border-b border-slate-100 shrink-0 flex justify-center items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-slate-300"></div>
                  <p className="text-[clamp(0.55rem,1.2vh,0.7rem)] font-bold text-slate-500 tracking-[0.2em] uppercase">Waiting Queue</p>
                  <div className="w-1.5 h-1.5 rounded-full bg-slate-300"></div>
                </div>
                <div className="w-full flex-1 min-h-0 p-1 2xl:p-2 flex flex-col items-center overflow-hidden">
                  {c.waitingTokens && c.waitingTokens.length > 0 ? (
                    <AutoScrollList>
                      {c.waitingTokens.map((wt, i) => (
                         <div key={i} className="text-[clamp(0.9rem,2vh,1.2rem)] font-bold text-slate-700 bg-slate-50 w-full text-center py-1 2xl:py-1.5 rounded-lg border border-slate-100 shadow-sm">
                          {wt.tokenLabel}
                        </div>
                      ))}
